...
// Drop-in for the landing page. No other imports from your project needed.
...
function PlanCard({
    ...
    return (
        <div
            className={`pricing-card h-full border bg-[var(--surface)] p-6 flex flex-col ${isFeatured ? 'border-[var(--gold)] ring-gold' : 'border-[var(--border)]'
                }`}
        >
            ...
            <button
                className={`mt-5 w-full rounded-xl px-4 py-2 font-medium ${isFeatured ? 'bg-white text-black' : 'btn btn-gold !w-full'
                    }`}
            >
        ...
        () => (
            <div className="mx-auto inline-flex items-center rounded-full border border-[var(--border)] bg-[var(--surface)] p-1">
                <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm ${currency === 'USD' ? 'bg-[var(--muted)] text-white' : 'text-[var(--subtle)]'}`}
                >
                    Global (USD)
                </button>
                <button
                    onClick={() => setCurrency('INR')}
                    className={`px-4 py-1.5 rounded-full text-sm ${currency === 'INR' ? 'bg-[var(--muted)] text-white' : 'text-[var(--subtle)]'}`}
                >
                    India (INR)
                </button>
            </div>
        ...
        <section className="py-14">
            <div className="mx-auto max-w-7xl px-4">
                <div className="text-center">
                    {Toggle}
                    <h2 className="mt-6 text-3xl md:text-4xl font-bold">Transparent plans for individuals, teams, and enterprises</h2>
                    <p className="mt-2 text-[var(--subtle)]">Centered, compact cards with all the details you need.</p>
                </div>

                <div className="mt-10 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
                    {PLANS.map((p) => (
                        <PlanCard key={p.key} plan={p} currency={currency} />
                    ))}
                </div>
            </div>
        </section>
