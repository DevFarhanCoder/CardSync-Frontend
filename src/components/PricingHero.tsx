// src/pages/PricingHero.tsx
// Same visuals; only the CTA buttons now respect auth.
  ...
  return (
    ...
    >
      ...
      <g transform="translate(180,50)">
        <rect x="0" y="0" rx="28" ry="28" width="260" height="420" fill="var(--surface)" stroke="var(--border)" strokeWidth="2" />
...
export default function PricingHero() {
  ...
  const onStartTrial = () => {
    // Hero promotes Pro trial; block guests, allow authed to billing with trial param
    if (!isAuthed) {
      navigate('/signin', { state: { from: location.pathname, intent: { type: 'trial', planId: 'pro' } } })
      return
    }
    navigate('/dashboard/billing?plan=pro&trial=1')
  }

  const onViewDemo = () => {
    // Keep this as a simple in-page link to the demo section
    ...
    <section className="relative overflow-hidden border-b border-[var(--border)] bg-[var(--bg)]">
      <div className="container grid items-center gap-12 py-16 md:py-24 lg:grid-cols-2">
        <div>
          ...
          {/* CTAs (logic only changed) */}
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button onClick={onStartTrial} className="btn btn-gold px-5 py-2.5">
              Start Free Trial
            </button>
            <button onClick={onViewDemo} className="btn btn-outline px-5 py-2.5">
              View Demo
            </button>
          </div>
        </div>
