...
import { X, Crown, Phone, MessageSquare, Copy, Settings, LogOut, MoreVertical, User as UserIcon } from "lucide-react";
import {
...
import { openDirect } from "@/lib/userApi";        // ✅ from (not frm)
...
export default function GroupRightPanel({ open, onClose, group }: Props) {
  ...
  const viewerIsAdmin = viewerIsOwner || admins.includes(String(viewerId));

  const sortedMembers = useMemo(() => {
    const meFirst = (a: UserLite, b: UserLite) => (String(a.id) === String(viewerId) ? -1 : String(b.id) === String(viewerId) ? 1 : 0);
  ...
  async function changePhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await refresh(); // reflect new photoUrl
  ...
  async function addSelectedToGroup() {
    ...
    setSaving(true);
    try {
      const phones = importResult.filter(m => selectedUserIds.includes(m.userId)).map(m => m.phone).filter(Boolean) as string[];
      ...
      <li className="py-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-neutral-400 truncate">{[m.email, m.phone].filter(Boolean).join(" • ")||"—"}</div>
          </div>

          {/* quick actions */}
          <div className="relative">
            <button className="rounded-lg bg-neutral-800 p-2" onClick={()=>setOpenMenu(v=>!v)} title="Actions">
              <MoreVertical className="h-4 w-4"/>
            </button>
            {openMenu && (
              <div className="absolute right-0 z-10 mt-2 w-44 rounded-lg border border-white/10 bg-neutral-900 text-sm shadow-xl">
                <button className="w-full px-3 py-2 text-left hover:bg-white/10 flex items-center gap-2" onClick={async()=>{
                  const { id } = await openDirect(m.id);
                  nav(`/dashboard/dm/${id}`);
                }}><MessageSquare className="h-4 w-4"/> Personal chat</button>
                {m.phone && <a className="block px-3 py-2 hover:bg-white/10 flex items-center gap-2" href={`tel:${m.phone}`}><Phone className="h-4 w-4"/> Call</a>}
                <button className="w-full px-3 py-2 text-left hover:bg-white/10 flex items-center gap-2" onClick={()=>nav(`/dashboard/user/${m.id}`)}><UserIcon className="h-4 w-4"/> View profile</button>
                {viewerIsAdmin && !isOwnerHere && (
                  <>
                    <button className="w-full px-3 py-2 text-left hover:bg-white/10 flex items-center gap-2" onClick={()=>makeAdmin(m.id, !isAdminHere)}><Crown className="h-4 w-4"/>{isAdminHere? "Remove admin":"Make admin"}</button>
                    <button className="w-full px-3 py-2 text-left hover:bg-white/10 text-red-300" onClick={()=>remove(m.id)}>Remove</button>
                  </>
                )}
              </div>
            )}
        ...
        <div className="p-3 space-y-4 overflow-auto h-[calc(100%-96px)]">
          <div className="flex items-center gap-4">
            ...
            <label className="cursor-pointer rounded-lg bg-neutral-800 px-3 py-2 text-sm hover:bg-neutral-700">
              <input type="file" accept="image/*" className="hidden" onChange={changePhoto} disabled={!viewerIsAdmin || uploading} />
          ...
          <div className="grid gap-2">
            ...
            <button onClick={saveBasic} className="w-max rounded-lg bg-yellow-400 px-4 py-2 text-black text-sm disabled:opacity-50" disabled={!viewerIsAdmin || saving}>
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
          ...
          <div className="rounded-xl border border-white/10 p-3">
            ...
            <button onClick={importFromGoogle} disabled={!viewerIsAdmin || importing} className="rounded-lg bg-neutral-800 px-3 py-2 text-sm">
