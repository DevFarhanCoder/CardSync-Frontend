...
export default function CompactPricingGrid({ defaultCurrency = 'INR' }: { defaultCurrency?: 'USD' | 'INR' }) {
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        {PLANS.map((p) => {
          ...
          return (
            <div
              key={p.key}
              className={`h-full rounded-2xl border bg-[var(--surface)] p-6 flex flex-col ${
            ...
            >
              ...
              <p className="text-[var(--subtle)] text-sm">{p.strap}</p>

              <div className="mt-5">
                <Price money={money} currency={currency} />
                {currency === 'INR' && p.commission && (
                  <div className="text-sm text-rose-400 mt-1">{p.commission}</div>
                )}
                <div className="text-xs text-[var(--subtle)]">{p.limit}</div>
              </div>
